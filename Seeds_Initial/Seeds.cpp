...
static float correl(const Image<byte>& im1, int i1,int j1,float m1,
    ...
    for (int u = - win; u <= win; u++) {
        for (int v = - win; v <= win; v++) {
            float I1 = (im1(i1 + u, j1 + v) - m1);
            float I2 = (im2(i2 + u, j2 + v) - m2);
            corr += I1 * I2;
            I1_norm += I1 * I1;
            I2_norm += I2 * I2;
        }
    }
    dist = corr / (sqrt(I1_norm) * sqrt(I2_norm) + EPS);
    return dist;
}
...
static void find_seeds(Image<byte> im1, Image<byte> im2,
    ...
    for(int y=win; y+win<maxy; y++) {
        ...
        for(int x=win; x+win<im1.width(); x++) {
            ...
            float maxNCC = -10.0f;
            int bestDisparity;
            for (int d = max(dmin, win-x); d <= min(dmax, im2.width()- (1+x+win)); d++) {
            ...
            if (maxNCC >= nccSeed) {
                ...
                Q.push(Seed(x, y, bestDisparity, maxNCC));
            }
        }
    }
    std::cout << std::endl;
        ...
        for(int i=0; i<4; i++) {
            ...
            if(0<=x-win && x+win<im1.width() && 0<=y-win && y+win<maxy &&
               ! seeds(x,y)) {
                ...
                float maxNCC = -10.0f;
                int bestDisparity;
                for (int d = max(dmin, s.d - 1); d <= min(s.d + 1, dmax) ; d++) {
                    if ((x + d - win < 0) || (x + d + win > im2.width() -1)) continue;
                    ncc = ccorrel(im1, x, y, im2, x + d, y);
                    if (ncc > maxNCC) {
                        maxNCC = ncc;
                        bestDisparity = d;
